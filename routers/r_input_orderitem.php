...
if (!empty($_POST['input_orderitem_validate'])) {
    ...
    $select = mysqli_query($conn, "SELECT * FROM tb_list_order WHERE menu='$menu' && orderan='$kode_order'");
    if (mysqli_num_rows($select) > 0) {
        $message = '<script>alert("The item entered already exists");
        ...
        $query = mysqli_query($conn, "INSERT INTO tb_list_order (menu, orderan, jumlah, catatan) values ('$menu', '$kode_order', '$jumlah', '$catatan' ) ");
